...
local DROPDOWNMENU_NAME_PREFIX = "AbinDropDownMenuXXX" -- Menu name prefix
...
function STDropDownMenu_GetHandle() return {
	...
	AddLine = function(self, ...)
		...
		--if (info["checked"]==false or checked) and not info["icon"] then
...
local DROPDOWNMENU_INSET_VERTICAL = 19 -- Menu vertical edge size
...
local function MenuButton_OnEnter(self)
	...
	if LastCloseButton and LastCloseButton ~= self.CloseButton then
		LastCloseButton:Hide();
	end
	...
	if not LastCloseButton or LastCloseButton ~= self.CloseButton then
		LastCloseButton = self.CloseButton
	end
	
	if not LastArrow[level] then
		
		LastArrow[level]={};
		LastArrow[level]["highlightTexture"]=self.highlightTextureH;
		LastArrow[level]["arrow"]=self.arrow;
		
	end
	
	local ARROW = LastArrow[level]["highlightTexture"];
	
	
	if ARROW ~= self.highlightTextureH then
		
		LastArrow[level]["arrow"]:SetVertexColor(ArrowColorL[1],ArrowColorL[2],ArrowColorL[3],ArrowColorL[4]);
		ARROW:Hide()
		LastArrow[level]["highlightTexture"]=self.highlightTextureH;
		LastArrow[level]["arrow"]=self.arrow;
	end
	...
	if not info.disabled then
		if not info.notClickable then
			...
			LastArrow[level]["highlightTexture"]:Show();
			LastArrow[level]["arrow"]:SetVertexColor(ArrowColorH[1],ArrowColorH[2],ArrowColorH[3],ArrowColorH[4]);
		end
	end
	
	for k,v in pairs(LastArrow) do
	
		if k > level then
			v["highlightTexture"]:Hide();
			v["arrow"]:SetVertexColor(ArrowColorL[1],ArrowColorL[2],ArrowColorL[3],ArrowColorL[4]);
		end
...
local function MenuButton_CloseButton_OnClick(self,info)
	...
	print(self.info.CloseButtonFun)
	...
	self:SetHeight(height)
		...
		if info.line then
			...
			if not self.line then
				self.line = CreateFrame("Button", nil, self)
			end
			
			
			local LineY = info.LineY or 0;
			
			self.line:SetPoint("TOPLEFT",0,height/2*-1 + LineY );
			self.line:SetPoint("TOPRIGHT")
			self.line:SetWidth(1);
			self.line:SetHeight(1);
		
			self.line:SetBackdrop({ bgFile = "Interface\\Tooltips\\UI-Tooltip-Background", tile = true, tileSize = 1, edgeFile = "Interface\\Tooltips\\UI-Tooltip-Background", edgeSize = 1, insets = {left = 1, right = 1, top = 1, bottom = 1 }, });
...
local function GetDropDownMenuButton(menu, id)
	...
	highlightTextureH:SetPoint("TOPLEFT");
	highlightTextureH:SetPoint("TOPRIGHT")
	--highlightTextureH:SetWidth(50);
	highlightTextureH:SetHeight(DROPDOWNMENU_BUTTON_HEIGHT);
...
local function CreateDropDownMenu(level)
	...
	if not OnMenuCloseButton then
		DROPDOWNMENU_INSET_VERTICAL = 12;
	end
