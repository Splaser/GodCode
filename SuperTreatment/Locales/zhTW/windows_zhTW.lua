...
function Lib.WowStHelperFrame()
	...
	local frame= WowStFrame.CreateMain("WowStHelperFrame",UIParent,"超級助手簡單模式");
	frame:SetWidth(200);
	frame:SetHeight(350);
...
function Lib.WowStVarFrame(parent)
	...
	frame.Edit.NameLabel:SetText("常量名稱:");
	...
	frame.Edit.TypeLabel:SetText("常量類型:");
	...
	frame.Edit.TypeRadio1Label:SetText("無值");
	...
	frame.Edit.TypeRadio2Label:SetText("單值");
	...
	frame.Edit.TypeRadio3Label:SetText("雙值");
	...
	frame.Edit.SizeLabel:SetText("編輯框寬度:");
	...
	frame.Edit.SizeTLabel:SetText(string.format("范圍: %s-%s 之間",frame.Edit.MinSize,frame.Edit.MaxSize));
	...
	frame.Edit.EditLabel:SetText("編輯框類型:");
	...
	frame.Edit.EditRadio1Label:SetText("數字型");
	...
	frame.Edit.EditRadio3Label:SetText("代碼型");
	...
	frame.Edit.SetTypeRadio=function(self)
		...
		elseif(self.Type==3)then
			...
			self.MinSize=20;
			self:SetSize();
			self.SizeTLabel:SetText(string.format("范圍: %s-%s 之間",self.MinSize,self.MaxSize));
		else
			self.TypeRadio1:SetChecked(0);
...
function Lib.Init()
	...
	--minimap:AddTooltipLine("超級助手")
	minimap:AddTooltipLine("超級助手核心插件  " .. GetAddOnMetadata("SuperTreatment", "Version"), 0, 1, 0, 1)
...
local function WowStHelperFrame_v_OnUpdate()
	...
	if GetTime() - StartTime>0.1 then
		...
		if SuperTreatmentSet["stop"] then
			SuperTreatmentFrame_stop:SetText("運行");
			...
			WowStHelperFrame.StartStop:SetText("運行");
		else
	...
	if WowStHelperFrame_v_OnUpdate_v then
		...
		wowam.Addons["SHelper"]["AddonsName"]="超級助手簡單模式";
		wowam.Addons["SHelper"]["name"]="超級助手簡單模式";
		wowam.Addons["SHelper"]["run"]="WowStHelperFrame_Show()";
		wowam.Addons["SHelper"]["inf"]="超級助手簡單模式";
		...
		wowam.Addons["SuperTreatment"]["AddonsName"]="超級助手正常模式";
		wowam.Addons["SuperTreatment"]["name"]="超級助手正常模式";
		wowam.Addons["SuperTreatment"]["run"]="SuperTreatment_Show()";
		wowam.Addons["SuperTreatment"]["inf"]="超級助手正常模式";
