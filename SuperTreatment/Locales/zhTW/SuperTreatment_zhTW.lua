...
ST_CastSpellFrame:RegisterEvent("UNIT_HEAL_PREDICTION"); -- 獲得治療目標
ST_CastSpellFrame:RegisterEvent("UNIT_SPELLCAST_SENT"); -- 施放目標技能
	...
	elseif V[1]=="select:on" or V[1]=="select:off" or V[1]=="select" then
		...
		if V[3] then
			...
			elseif b1 and (b1 ~= "off" or b1 ~= "on") then
				--print("技能的開關參數錯誤!請選 on/off");
				stprint("|cff00ff00" .. a .. "方案下的|cffff0000[|r" .. b.."|cffff0000]技能的開關參數錯誤!請選 on/off")
				return;
			end
		end
	...
	elseif V[1]=="MT" then
		...
		if V[3] then
			...
			if name and V[2] and tonumber(V[2]) and tonumber(V[2])>=1 and tonumber(V[2])<=8 then
				...
				stprint("成功設定 MT"..V[2].." 為:" .. name);
				 SuperTreatment["Menu"]["Main"]["DropDownMenu"]:Refresh(3);
			else
				stprint("|cffff0000MT命令格式錯誤或沒當前目標！");
			end
...
function SuperTreatment_commaninf()
	...
	print("|cffffff00超級助手宏命令:")
	print("|cffff0000重置窗口:|r /st reset")
	
	print("|cffff0000顯示/隱藏插件:|r /st show")
	print("|cffff0000自動施放(啟動/停止):|r /st startstop")
	print("|cffff0000手動施放:|r /st run")
	print("|cffff0000啟動自動施放:|r /st auto")
	print("|cffff0000停止自動施放:|r /st stop")
	
	print("|cffff0000加載方案:|r /st load 方案名稱")
	print("|cffff0000選擇/放棄施法方案:|r /st select 施法方案名稱")
	print("|cffff0000選擇/放棄施法方案下的技能:|r /st select 施法方案名稱 技能序號")
	
	print("|cffff0000選擇施法方案:|r /st select:on 施法方案名稱")
	print("|cffff0000放棄施法方案:|r /st select:off 施法方案名稱")
	
	print("|cffff0000選擇施法方案及其下的技能:|r /st select:on 施法方案名稱 技能序號:on")
	print("|cffff0000放棄施法方案及其下的技能:|r /st select:off 施法方案名稱 技能序號:off")
	
	print("|cffff0000施法方案及其下的技能(on/off):|r /st select:on/off 施法方案名稱 技能序號:on/off")
	print("|cffff0000施法方案及其下的技能(on/off):|r /st select:on/off 施法方案名稱 技能序號:on/off")
	
	print("|cffff0000手動執行方案:|r /srun 方案名稱")
	
	print('|cffff0000用函數 stcommand 來替代|r /st 如: stcommand("show") 顯示/隱藏插件')
	
	print("|cffff0000設定當前目標為全局MT目標:|r /st MT 編號 如: /st MT 1")
	print("|cffff0000取消全局MT目標:|r /st MT 編號 off  如: /st MT 1 off")
...
local function RaidList()
	...
	if not RDB["target"] then	
		...
		RDB["mouseovertarget"]["name"]="鼠標目標的目標";
...
local function UpUnitFrame_OnEvent(self, event, ...)
	...
	elseif ( event == "COMBAT_TEXT_UPDATE" )  then
            ...
            if (arg1 == "SPELL_ACTIVE") then
				...
				--if GetSpellInfo(SpellIds["復仇(防御姿態)"]) == arg2 then
				--print(arg2)
				AnalysisInf(arg2);
			end
...
local function Initialization()
	...
	BINDING_HEADER_SuperTreatment = "超級助手";
	BINDING_NAME_SuperTreatment_SHOW = "顯示/隱藏";
	BINDING_NAME_SuperTreatment_STARTSTOP = "自動施放(啟動/停止)";
	BINDING_NAME_SuperTreatment_RUN = "手動施放";
	BINDING_NAME_SuperTreatment_AUTO = "啟動自動施放";
	BINDING_NAME_SuperTreatment_STOPALL = "停止自動施放";
	BINDING_NAME_SuperTreatment_BURSTS = "連發(按下啟動反之停止)"
	
	BINDING_NAME_SuperTreatment_CastProgram1 = "選擇/放棄 施法方案 1";
	BINDING_NAME_SuperTreatment_CastProgram2 = "選擇/放棄 施法方案 2";
	BINDING_NAME_SuperTreatment_CastProgram3 = "選擇/放棄 施法方案 3";
	BINDING_NAME_SuperTreatment_CastProgram4 = "選擇/放棄 施法方案 4";
	BINDING_NAME_SuperTreatment_CastProgram5 = "選擇/放棄 施法方案 5";
	
	BINDING_NAME_SuperTreatment_CastProgram6 = "選擇/放棄 施法方案 6";
	BINDING_NAME_SuperTreatment_CastProgram7 = "選擇/放棄 施法方案 7";
	BINDING_NAME_SuperTreatment_CastProgram8 = "選擇/放棄 施法方案 8";
	BINDING_NAME_SuperTreatment_CastProgram9 = "選擇/放棄 施法方案 9";
	BINDING_NAME_SuperTreatment_CastProgram10 = "選擇/放棄 施法方案 10";
	
	BINDING_NAME_SuperTreatment_CastProgram11 = "選擇/放棄施法方案 11";
	BINDING_NAME_SuperTreatment_CastProgram12 = "選擇/放棄施法方案 12";
	
		
	BINDING_NAME_SuperTreatment_RunProgram1 = "手動執行施法方案 1";
	BINDING_NAME_SuperTreatment_RunProgram2 = "手動執行施法方案 2";
	BINDING_NAME_SuperTreatment_RunProgram3 = "手動執行施法方案 3";
	BINDING_NAME_SuperTreatment_RunProgram4 = "手動執行施法方案 4";
	BINDING_NAME_SuperTreatment_RunProgram5 = "手動執行施法方案 5";
	
	BINDING_NAME_SuperTreatment_RunProgram6 = "手動執行施法方案 6";
	BINDING_NAME_SuperTreatment_RunProgram7 = "手動執行施法方案 7";
	BINDING_NAME_SuperTreatment_RunProgram8 = "手動執行施法方案 8";
	BINDING_NAME_SuperTreatment_RunProgram9 = "手動執行施法方案 9";
	BINDING_NAME_SuperTreatment_RunProgram10 = "手動執行施法方案 10";
	
	BINDING_NAME_SuperTreatment_RunProgram11 = "手動執行施法方案 11";
	BINDING_NAME_SuperTreatment_RunProgram12 = "手動執行施法方案 12";
...
function SuperTreatment_OnUpdate(arg1)
	...
	if not RaidList_OK then
		...
		local V = "|cFF8000FF超級助手|cff00ffff" .. GetAddOnMetadata("SuperTreatment", "Version")
		print(V);
		print("|cFF8000FF超級助手幫助命令:|cff00ffff /st");
		print("|cffff0000超級助手: |cffffff00請連接客戶端如果還失敗請更新！")
	end
	--[[
...
if not amisActivePet then
	function amisActivePet(v)-- 寵物狀態按鈕
...
if not amautoCastEnabledPet then
	function amautoCastEnabledPet(v)-- 寵物技能是否能激活狀態
...
if not amautoCastAllowedet then
	function amautoCastAllowedet(v)-- 寵物技能是否能激活
	...
	if SuperTreatmentDBF["AddOnMemory"] and SuperTreatmentDBF["AddOnMemory"]["Leftfighting"] then
	   ...
	   if event == "PLAYER_ENTERING_WORLD" then 
		  ...
		  if SuperTreatmentDBF["AddOnMemory"]["inf"] then
			print(format("|cff66C6FF%s:|r %s","回收內存",formatmem(before - gcinfo()))) 
		  end
...
local function RUNTEST()
	...
	coroutine.yield();
	am_index[381]=0
	print(am_index[382],GetTime()-TT1)
end
...
local function ST_RUN_OnUpdate(arg1)
	...
	if (not thread or (thread and coroutine.status(thread) == "dead")) and  not SuperTreatmentSet["stop"] and GetTime() - ST.ForTime >0.03 then
		...
		if SuperTreatmentInf.SpellsRun and isSuperTreatmentload then
				...
				--thread = coroutine.create(RUNTEST);
	...
	if thread and coroutine.status(thread) ~= "dead" then
		...
		if am_index[381]==1 or GetTime()-TT1 > 1 then
...
local function GetEP(v,i)
	...
	if text then
		if tbl.checked then
			text = "    "..i ..".|cff00ffff"..text .. "|r  |cff00ff00啟用！";
		else
