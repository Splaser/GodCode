...
function addon:OnMenuRequest(level, value, menu, MenuEx)
	...
	if level == 1 then -- 1級菜單內容
		...
		menu:AddLine("text", "|cffffff00啟用信息收集","checked",tbl["checked"],"func", "SetTbl","arg1", self,
		"arg2",{tbl,"checked",not tbl["checked"],level}
		);
		
		menu:AddLine("text", "|cffffff00刪除全部信息", "func", "BuffListAllDel", "arg1", self,"arg2",MenuEx.ExternalData[1],
		"hasConfirm", 1, "confirmText", "是否刪除刪除全部信息?"
		);
		...
		menu:AddLine("text", "|cffffff00過濾關鍵字","hasEditBox", 1, "editBoxText", addon.SpellBuffInformationFiltering_value, "editBoxFunc", "SpellBuffInformationFiltering", "editBoxArg1", self,"tooltipText",str,"tooltipTitle","信息");
		
		
		
		tbl["type"] = tbl["type"] or -1000;
		
		menu:AddLine("text", "|cffffff00顯示自己","isRadio", 0,"checked",tbl["type"] == -999,
		"func", "SetTbl","arg1", self,"arg2",{tbl,"type",-999,level})
		menu:AddLine("text", "|cffffff00顯示全部","isRadio", 0,"checked",tbl["type"] == -1000,
...
function addon:SetMenu(Value)
	...
	elseif IsControlKeyDown()  and not IsAltKeyDown() then
		...
		print("|cffff0000圖標:|r " .. tbl[index]["icon"]);
		return;
